...
render() { local slug="$1"
  "$CHROME" --headless=new --disable-gpu --hide-scrollbars --no-first-run --force-device-scale-factor=1 \
    --window-size=1200,675 --user-data-dir="$GEN/.c-$slug" --screenshot="$ROOT/f09-$slug.png" "file:///$GEN/$slug.html" >/dev/null 2>&1
  rm -rf "$GEN/.c-$slug"; echo "  rendered f09-$slug.png ($(wc -c < "$ROOT/f09-$slug.png" 2>/dev/null) bytes)"
}

IC_CITY='<path d="M-46 38 L-46 -8 L-32 -8 L-32 -28 L-16 -28 L-16 -42 L0 -42 L0 -28 L16 -28 L16 6 L32 6 L32 -10 L46 -10 L46 38 Z"/><line x1="-46" y1="38" x2="46" y2="38"/>'
...
IC_BALANCE='<path d="M-44 -22 L0 -32 L44 -22"/><path d="M0 -32 L0 32"/><path d="M-44 -22 L-30 6 a14 14 0 0 0 28 0 Z"/><path d="M44 -22 L30 6 a14 14 0 0 0 28 0 Z"/><line x1="-12" y1="32" x2="12" y2="32"/>'
IC_GRID='<rect x="-42" y="-42" width="36" height="36" rx="4"/><rect x="6" y="-42" width="36" height="36" rx="4"/><rect x="-42" y="6" width="36" height="36" rx="4"/><rect x="6" y="6" width="36" height="36" rx="4"/>'
IC_HOUSE='<path d="M-44 -2 L0 -40 L44 -2 V38 H-44 Z"/><rect x="-12" y="6" width="24" height="32"/><line x1="0" y1="-40" x2="0" y2="-2"/>'

echo "Building Folder 9 featured images..."
EB="Abu Dhabi Careers"
page_html "$EB" "Abu <span class=alt>Dhabi</span>" "The quieter cousin worth knowing" "$IC_CITY" > "$GEN/abu-dhabi-careers.html"
page_html "$EB" "Finding <span class=alt>A Job</span>" "The capital's hiring reality" "$IC_FIND" > "$GEN/finding-a-job.html"
page_html "$EB" "Work <span class=alt>Permits</span>" "Process and ADGM nuances" "$IC_PASSPORT" > "$GEN/work-permits-and-visas.html"
page_html "$EB" "Capital <span class=alt>Salaries</span>" "What expatriates earn" "$IC_RIYAL" > "$GEN/salaries.html"
page_html "$EB" "Government <span class=gold>vs</span> <span class=alt>Private</span>" "The choice most expats get wrong" "$IC_BALANCE" > "$GEN/government-vs-private.html"
page_html "$EB" "Hiring <span class=alt>Sectors</span>" "Where the demand really sits" "$IC_GRID" > "$GEN/sectors.html"
page_html "$EB" "Relocating <span class=alt>Here</span>" "What the move really feels like" "$IC_HOUSE" > "$GEN/relocating-here.html"

for s in abu-dhabi-careers finding-a-job work-permits-and-visas salaries government-vs-private sectors relocating-here; do render "$s"; done
echo "Done."
