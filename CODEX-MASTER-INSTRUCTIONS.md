...
**Scope:** The full InspireAmbitions portfolio (jobstrike.ae deferred), one strategy, one design system.
...
calculators), build **sixteen workstreams across the portfolio**, in this order:
...
**Distribution / network layer (5 briefs — folder `distribution-battleplan/`, covers all other properties):**
11. **Brief 01 — Network shell:** shared cross-link strip on every property, resume.→cv.
    consolidation (301s), unified PostHog, SEO hygiene, zero-dead-end rule.
12. **Brief 02 — Tools hub:** gratuity-calculator SEO empire (~40+ scenario pages, share cards),
    JD generator with employer-leads capture, salary & notice-period micro-tools.
13. **Brief 03 — Decision tool:** "Should I Take This Dubai Job?" as a first-class app with
    shareable verdict cards, ask-your-people polls, and salary-question SEO pages.
14. **Brief 04 — Content engine [mostly WP]:** blog permalink surgery, wrapper→landing upgrades,
    four topic clusters anchored on tools.
15. **Brief 05 — Newsletter flywheel:** segmented Resend audiences, weekly featured-tool memo,
    subscribe inlets everywhere, webhook→PostHog measurement.

**Cross-product (implicit in the above):**
16. One design system, one analytics plane (PostHog org "Job Strike"), one email platform
    (Resend), the task→CV→jobs funnel, and the network rule: every page on every property
    routes onward — no dead ends anywhere.
...
# ============================================================
# PRODUCT 3: DISTRIBUTION NETWORK — FULL BRIEFS (distribution-battleplan/)
# ============================================================
